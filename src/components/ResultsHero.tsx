...
interface ResultsHeroProps {
  ...
  reportMode?: boolean;
  realEstValue?: string;
  realSignalScore?: string;
  realConfidence?: string;
}
...
export default function ResultsHero({
  ...
  // In report mode, real values (passed from ResultsSection) always replace
  // the placeholder defaults above. The defaults only ever show on the
  // pre-report (locked/blurred) screen.
  const displayStats = reportMode
    ? [
        { ...stats[0], value: realEstValue ?? stats[0].value },
        { ...stats[1], value: realSignalScore ?? stats[1].value },
        { ...stats[2], value: realConfidence ?? stats[2].value },
      ]
    : stats;
  ...
  return (
    <section className="relative z-10 pt-[15vh]">
      <div className="mx-auto flex w-full max-w-[1440px] px-4 md:px-6 xl:px-10 flex-col items-center text-center">
        ...
        {/* ── Headline ── */}
        <h1 className={`${playfair.className} px-2 text-[30px] font-semibold leading-[1.15] text-[#0B1E33] sm:text-[38px] md:text-[44px]`}>
          {reportMode ? (
            <>
        ...
        >
          <div className="flex min-w-0 flex-1 items-center gap-2.5 px-4 py-2.5 sm:px-5">
            <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white/10">
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" aria-hidden="true">
                <path d="M12 21s-7-5.8-7-11a7 7 0 1114 0c0 5.2-7 11-7 11z" strokeLinecap="round" strokeLinejoin="round" />
                <circle cx="12" cy="10" r="2.5" />
              </svg>
            </div>
            <span className={`${inter.className} min-w-0 flex-1 truncate text-[13px] font-medium text-white/90`}>
              {location}
            </span>
          </div>
          ...
          {reportMode && (
            <div className="flex shrink-0 items-center gap-2 border-l border-white/[0.10] bg-[#1FAE9F]/10 px-3 py-2.5 sm:px-4">
              ...
              <span className={`${inter.className} whitespace-nowrap text-[11px] font-semibold uppercase tracking-[0.14em] text-[#1FAE9F]`}>
                Live Data
              </span>
            </div>
          )}
